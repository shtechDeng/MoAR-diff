...
import numpy as np
from module import Attention, PreNorm, FeedForward, Upsample
...
class ViViT(nn.Module):
    def __init__(self, image_size, patch_size, num_frames, depth=3, heads=3,
        ...
        #     if i == depth+1:
        #         space_dims.append(num_patches)
        #     else:
        #         space_dims.append(num_patches*num_frames)
        ...
        self.conv3d = nn.Sequential(
            ...
            #Rearrange('b (p1 p2 p3) d -> b d p1 p2 p3',
                    #   p1=patch_size[0], p2=patch_size[1], p3=patch_size[2], d=d, h=h, w=w),
            Rearrange('b (p1 p2 p3) (d h w c) -> b (d h w c) p1 p2 p3',
                      p1=patch_size[0], p2=patch_size[1], p3=patch_size[2], d=d, h=h, w=w),
            nn.Conv3d(in_channels=space_dims[-1], out_channels=64, kernel_size=3, padding="same")
        )
...
if __name__== "__main__" :
    b = torch.ones(12, 4, 1, 160, 192, 160)
    vit = ViViT(image_size=(160, 192, 160), patch_size=(10, 12, 10), num_frames = 4)
    out = vit(b)
